...
#pragma region movs
// Movs (e.g. mov ax, bx) DONE (//TODO(radu): REFACTOR THE CODE (TO MUCH REPEATING))
...
void mov_imm_reg(rcvm_t *vm, uint8 *instr_byte) //DONE(radu)
{
...
void mov_mem_acc(rcvm_t *vm, uint8 *instr_byte) //DONE(radu)
{
	const bool W_FIELD = (*instr_byte) & 1;
	...
	switch (MOD_FIELD)
	{
		case 0b00:
		{
			if (D_FIELD)
			{
				if (RM_FIELD == 0b110)
				{
					...
					printf("add %s, [%d]\n", rcvm_get_reg_name(REG_FIELD, W_FIELD),
							disp);
					const uint8 add_value = rcvm_reg_read(vm, REG_FIELD) + rcvm_mem_read(vm, disp);
				...
				else
				{
					printf("add %s, [%s]\n", rcvm_get_reg_name(REG_FIELD, W_FIELD),
							rcvm_get_disp_name(RM_FIELD));
					const uint8 add_value = rcvm_reg_read(vm, REG_FIELD) + rcvm_mem_read(vm, rcvm_get_disp(vm, RM_FIELD));
					rcvm_reg_write(vm, REG_FIELD, add_value);
		...
		case 0b01:
		{
			...
			if (D_FIELD)
			{
				printf("add %s, [%s %c %d]\n", rcvm_get_reg_name(REG_FIELD, W_FIELD),
						rcvm_get_disp_name(RM_FIELD), (disp < 0) ? '-' : '+', abs(disp));
				uint8 add_value = rcvm_reg_read(vm, REG_FIELD) + rcvm_mem_read(vm, rcvm_get_disp(vm, RM_FIELD) + disp);
				rcvm_reg_write(vm, REG_FIELD, add_value);
			}
			else
			{
		...
		case 0b11:
		{
			if (D_FIELD)
			{
				printf("add %s, %s\n", rcvm_get_reg_name(REG_FIELD, W_FIELD),
						rcvm_get_reg_name(RM_FIELD, W_FIELD));
				uint8 add_value = rcvm_reg_read(vm, REG_FIELD) + rcvm_reg_read(vm, RM_FIELD);
				rcvm_reg_write(vm, REG_FIELD, add_value);
			}
			else
			{
				printf("add %s, %s\n", rcvm_get_reg_name(RM_FIELD, W_FIELD),
						rcvm_get_reg_name(REG_FIELD, W_FIELD));
				uint8 add_value = rcvm_reg_read(vm, RM_FIELD) + rcvm_reg_read(vm, REG_FIELD);
...
void add_imm_acc(rcvm_t *vm, uint8 *instr_byte)
{
	...
	const uint8 add_value = rcvm_reg_read(vm, REG_A) + data;
